...
const SurveyChart = () => {
  ...
  const [filter, setFilter] = useState<AnswerType>({});
  ...
  const toggleFilter = (questionIndex: string, itemIndex: number) => {
    let _filter = { ...filter };
  ...
  let filteredAnswer = answer.filter((item) => {
    ...
    for (let i = 0; i < keys.length; i++) {
      if (filter?.[keys[i]] !== item?.[Number(keys[i])]) return false;
    }
    return true;
  });
